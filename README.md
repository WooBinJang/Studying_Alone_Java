...
# 객체
- 자기만의 특성을 가지고 있으며 다른 것과 식별 가능한 것

# 객체 지향 프로그래밍(OOP)
- 부품에 해당하는 객체들을 먼저 만들고 이것들을 하나씩 조립 및 연결해서 전체 프로그램을 완성하는 프로그램을 개발하는 기법
...
# 자바 클래스 라이브러리 API차이? 
- 클래스 : 객체를 생성하기 위한 필드와 메소드가 정의 된것  
- 라이브러리 : 프로그램 개발에 자주 쓰이는 클래스 및 인터페이스 모음  구현 로직이어야만 호출가능 자바 같은경우 ex) import ...
- API :  프로그램 개발에 자주 사용되는 자바에서 제공하는 클래스 및 인터페이스 모음 구현 로직이 없이 호출 가능 ex) Math.abs()
...
# 프로세스(process)
- 애플리케이션을 실행하면 운영체제로 실행에 필요한 메모리를 할당받아 애플리케이션이 되는데 이것을 프로세스라고 한다.

# 스레드(thread)
- 프로세스 내에서 작업을 처리하기 위한 코드 실행 흐름
- 프로세스가 시작되면 메인 스레드가 생성되고, 프로세스를 시작하게 된다.
- start() 호출해야만 새로운 흐름이 시작된다.

# 멀티 스레드
- 하나의 프로세스내에 두 개 이상의 스레드가 있을 경우
- 각각의 스레드는 해당 작업을 처리하기 위해 별도의 코드 실행 흐름을 생성
- 프로세스 내에서 실행 중인 스레드가 하나라도 있으면 프로세스는 종료되지 않음( main 스레드가 종료 되었다고 해서 프로세스가 종료되는 것은 아님)

# 메인스레드
- 모든 자바 애프리케이션은 메인 스레드가 main() 메소드를 실행하면서 시작됨
...
# 스레드의 이름
- 기본값은 Thread-n
- setName()으로 설정 시 이름 설정 가능
- getName()으로 스레드 이름 확인
